...
squares = [x**2 for x in xrange(9)]
cubes = [x**3 for x in xrange(6)]
squares_of_evens = [x**2 for x in xrange(9) if x % 2 == 0]

def palindromes(strings):
	return [s + s[::-1] for s in strings]

def starting_with(letter, names):
	return [name for name in names if name.lower().startswith(letter.lower())]
